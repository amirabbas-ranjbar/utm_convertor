...
def convertor(input):
    ...
    #  zone = north / Math.Pow(10, (Math.Floor(Math.Log10(north) + 1)) - 2);

    zone = int((north) / math.pow(10, (math.floor(math.log10(north) + 1)) - 2))

    # print(utmToLatLng(zone,east,north,True))
    #convertedData =(subId)
    convertedData = utmToLatLng(zone, east, north, True)
    new = convertedData + (subId,)
    return new
